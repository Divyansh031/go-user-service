...
func (db *ScyllaDB) CreateUser(ctx context.Context, user *domain.User) error {
	...
	emailExists, err := db.CheckEmailExists(ctx, user.Email)
	if err != nil {
		return err
	}
	if emailExists {
	...
	phoneExists, err := db.CheckPhoneExists(ctx, user.PhoneNumber)
	if err != nil {
		return err
	}
	if phoneExists {
	...
	// Insert into phone lookup table
...
func (db *ScyllaDB) GetUserByID(ctx context.Context, id uuid.UUID) (*domain.User, error) {
...
func (db *ScyllaDB) UpdateUser(ctx context.Context, user *domain.User) error {
	...
	// Update lookup tables if contact changed
...
func (db *ScyllaDB) DeleteUser(ctx context.Context, id uuid.UUID) error {
...
// Helper methods
func (db *ScyllaDB) insertPhoneLookup(ctx context.Context, phone string, userID uuid.UUID) error {
...
func (db *ScyllaDB) insertEmailLookup(ctx context.Context, email string, userID uuid.UUID) error {
